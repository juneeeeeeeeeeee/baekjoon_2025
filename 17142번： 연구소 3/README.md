# 17142번: 연구소 3 - <img src="https://static.solved.ac/tier_small/13.svg" style="height:20px" /> Gold III
...
<p>V개의 바이러스 중 M개를 활성화해야 하는데, 이를 위해 V개 중 M개를 고르는 조합을 이용했다. 
BFS를 이용했으며, 처음에는 각 바이러스에 대해 큐를 놓고 시간 1틱당 큐를 돌리는 방법을 사용했으나 그럴 필요가 없으므로 모든 위치를 큐에 집어넣고 1틱마다 이전 큐의 원소들을 모두 pop한 뒤 다음 원소들을 push하는 방식으로 진행하였다. 

  
<p>큐가 다 비지 않아도 퍼뜨리기가 완료된 경우가 있으므로 주의해야 한다. (비활성화된 바이러스를 마지막에 탐색하는 경우)
바이러스가 모두 퍼진 것을 확인하기 위해 처음에는 map 모든 원소를 확인하는 방법을 생각했으나 시간 초과가 나서, 빈 칸의 개수를 맨 처음에 모두 세고 이를 하나씩 빼는 방식으로 구현했다. 
